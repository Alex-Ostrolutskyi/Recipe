...
class RecipeView extends View {
  ...
  addHandlerRender(data) {
    ...
    event.forEach(el => window.addEventListener(el, data))
  }

  addHandlerUpdateServings(data) {
    this._parentEl.addEventListener("click", function (e) {
      const btnEl = e.target.closest(".btn--increase-servings")
      if (!btnEl) return
      const updateTo = +btnEl.dataset.updateTo
      if (updateTo > 0) data(updateTo)

    })
  }

  addBookmarkHandler(data) {
    this._parentEl.addEventListener("click", function (e) {
      const btnBookmark = e.target.closest(".btn-bookmarks")
        ...
        <img src="${this._data.image}" alt="${this._data.title}" class="recipe__img" />
          ...
          <div class="recipe__details">
            <div class="recipe__info">
              ...
              <span class="recipe__info-data recipe__info-data--minutes">${this._data.coockingTime}</span>
          ...
          <div class="recipe__ingredients">
            ...
            <ul class="recipe__ingredient-list">
              ${this._data.ingredients.map(this._generateMarkupIngrediend).join("")}
            </ul>
          </div>
